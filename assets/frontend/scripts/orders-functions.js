...
function foodbakery_update_order_time( thisObj, order_id, admin_url ){
    jQuery('#order-det-'+ order_id +' .btn-print').prop('disabled', true);
    jQuery('.order-status-loader-'+ order_id +'').html('<img src="' + foodbakery_globals.plugin_dir_url + 'assets/frontend/images/ajax-loader.gif" alt="loader">');
    jQuery.ajax({
        type: "POST",
        url: admin_url,
        dataType: 'json',
        data: 'action=foodbakery_update_order_time&order_id='+ order_id +'&order_time='+ thisObj.value,
        success: function ( response ) {
            jQuery('.order-status-loader-'+ order_id +'').html('');


            jQuery(document).find('#delivary_time_'+ order_id).text(response.order_time);
