    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if(response.type != "error"){
                jQuery(".login-form .create- a").trigger("click");
                var msg = '<div class="growl growl-success growl-medium"><div class="growl-close">×</div><div class="growl-title"></div><div class="growl-message">'+response.msg+'</div></div>';
                jQuery("p.foodbakery-dev-login-top-msg").append(msg).show();
            }
        }
    });
}
...
jQuery(document).on("click", "#profile_form", function () {
    ...
    if($(this).attr('dis')==='1'){
       ...
       // foodbakery_show_response('Click on Location search button', '', thisObj);

       alert('Address is wrong ! or press location search button');
            ...
            var publicfunc = {
                ...
                init: function (object) {
                    ...
                    google.maps.event.addListener(_self.vars.marker, 'dragend', function (event) {
                        ...
                        console.log('dragend function.js');
                    });
    ...
    $(document).on('click','.sa_location', function(){
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                $(document).find('#foodbakery_post_loc_latitude').val(position.coords.latitude);
                $(document).find('#foodbakery_post_loc_longitude').val(position.coords.longitude);

               //ar location = `https://maps.googleapis.com/maps/api/geocode/json?latlng=`+position.coords.latitude+`,`+position.coords.longitude+`&key=AIzaSyCbIoEdTP3GJA88AtDlyvIoETCIVnKp1bU`;
