...
$(document).on("keyup", ".dev-menu-search-field", function () {
  ...
  $.ajax({
    ...
    data:
      ...
      "&_menu_keyword=" +
      keyword_val +
      "&action=restaurant_detail_menu_search",
    dataType: "json",
  })
...
$(document).on("change", 'input[name="order_fee_type"]', function () {
  ...
  var selected_type_label = $('input[name="order_fee_type"]:checked').data(
    "label"
  );
  ...
  $.ajax({
    ...
    data:
      ...
      "&_rid=" +
      _rid +
      "&action=foodbakery_restaurant_set_fee_type",
    dataType: "json",
  })
    .done(function (response) {})
    .fail(function () {});
...
$(document).on("click", ".restaurant-add-menu-btn", function () {
  ...
  if (!_this.hasClass("is-disabled")) {
    ...
    foodbakery_show_loader(
      ".restaurant-add-menu-btn-" + _this_id + "",
      "",
      "button_loader",
      thisObj
    );
    ...
    selecting_type = $.ajax({
      ...
      data:
        ...
        "&_rid=" +
        _this_rid +
        "&extra_atts=&action=foodbakery_restaurant_add_menu_item",
      dataType: "json",
    })
      .done(function (response) {
        //this_loader.html('');
        ...
        if (typeof response.li_html !== "undefined" && response.li_html != "") {
          $(".dev-menu-orders-list")
            .find(".categories-order")
            .append(response.li_html);
...
$(document).on("click", ".dev-remove-menu-item", function () {
  ...
  if (!_this.hasClass("is-disabled")) {
    ...
    this_loader.html(
      '<img src="' +
        plugin_url +
        'assets/frontend/images/ajax-loader.gif" alt="">'
    );
    selecting_type = $.ajax({
      url: ajax_url,
      method: "POST",
      data:
        ...
        extra_arr.toString() +
        "&position=" +
        position_arr.toString() +
        "&_id=" +
        _this_index +
        "&_rid=" +
        _this_rid +
        "&action=foodbakery_restaurant_remove_menu_item",
      //dataType: "json"
    })
      .done(function (response) {
...
jQuery(document).on(
  "change",
  ".extras-detail-att input[type='checkbox']",
  function () {
    var thisObj = jQuery(this);
    var total_required = thisObj
      .closest(".extras-detail-main")
      .find("input[name='required_count']")
      .val();
    var checked_values = thisObj
      .closest(".extras-detail-main")
...
$(document).on("click", ".sa_increment", function () {
  ...
  var total_count = parseInt(
    thisObj.parents().eq(6).find(".total_count").attr("total_count")
  );
    ...
    if (these.is(":checked")) {
      ...
      if (max_qty !== "") {
        ...
        if (total_qty > max_qty) {
          alert(
            "Only " + sub_quntity + " " + title + " Products are available"
          );
          flag = false;
          return;
        }
  ...
  if (flag) {
    ...
    thisObj
      ...
      .eq(6)
      .find(".total_count")
      .attr("total_count", price.toFixed(2));
    thisObj
      .parents()
      .eq(6)
      .find(".total_count")
      .text("Total : " + price.toFixed(2) + " €");
  }
...
$(document).on("click", ".add-extra-menu-btn", function () {
  ...
  if (total_extras > 0) {
    ...
    $.each(extras_con, function (index, element) {
      var required_count = Number(
        $(this).find('input[name="required_count"]').val()
      );
    ...
    if (false === extra_check_f) {
      ...
      $.each(extra_name_arr, function (index, element) {
        var this_extr_name = main_con.find(
          'input[name="extra-' + element + '"]:checked'
        );
      ...
      if (!_this.hasClass("is-disabled")) {
        ...
        var sa_quantity_ary = [];
        jQuery.each(
          jQuery('.extras-detail-options input[type="checkbox"]:checked'),
          function (index, e) {
            var ind = $(e).data("ind");
        ...
        jQuery.each(
          jQuery('.extras-detail-options input[type="radio"]:checked'),
          function (index, e) {
            var ind = $(e).data("ind");
            var ex_tittle = $(e).attr("name");
            extra_atts_ary.push(ind);
        ...
        $.ajax({
          ...
          data:
            ...
            "&extra_notes=" +
            extra_notes +
            "&action=foodbakery_restaurant_add_menu_item",
          dataType: "json",
        })
          .done(function (response) {
            this_loader.html("");
            foodbakery_show_response(response, "", thisObj);
            if (
              typeof response.li_html !== "undefined" &&
              response.li_html != ""
            ) {
              if ("true" == is_updating) {
...
$(document).on("click", ".sa_extra_checkbox", function () {
  ...
  var quantity = parseInt(
    these.closest(".modal-content").find(".sa_quantity_in").val()
  );
    ...
    if (theses.is(":checked")) {
      ...
      if (max_qty === "") {
        ...
        if (total_qty > Number(max_qty)) {
          alert(
            "Only " + sub_quentity + " " + title + " Products are available"
          );
...
$(document).on(
  ...
  function () {
    ...
    if (total_extras > 0) {
      var extra_check_f = true;
      $.each(extras_con, function (index, element) {
        var this_extr = $(this).find(
          'input[name="extra-' + index + "-" + _this_menu_id + '"]'
        );
        if (!this_extr.is(":checked")) {
          extra_check_f = false;
        }
...
$(document).on("contentchanged", ".dev-menu-orders-list", function () {
  ...
  var menu_fee_pr = $(".dev-menu-orders-list")
    .find(".dev-menu-charges")
    .attr("data-confee");
  var menu_fee_pr_noc = $(".dev-menu-orders-list")
    .find(".dev-menu-charges")
    .attr("data-fee");
  ...
  var vat_switch = $(".dev-menu-orders-list")
    .find(".dev-menu-price-con")
    .attr("data-vatsw");
  var vat_perc = $(".dev-menu-orders-list")
    .find(".dev-menu-price-con")
    .attr("data-vat");
  ...
  if (vat_switch == "on" && subtotal > 0) {
    ...
    $(".dev-menu-orders-list")
      .find(".dev-menu-vtax")
      .html(cs_number_format(vat_price));
  ...
  $(".dev-menu-orders-list")
    .find(".dev-menu-grtotal")
    .html(cs_number_format(subtotal));

  var cartcount = jQuery("#count_cart_items")
    .children(".categories-order")
    .children("li").length;
...
$(document).on("click", ".menu-order-confirm", function () {
  ...
  $(".menu-order-confirm").html(
    btn_text +
      ' <div class="foodbakery-button-loader" style="display: block;"><div class="spinner"><div class="double-bounce1"></div><div class="double-bounce2"></div></div></div>'
  );
  ...
  if (
    $(".dev-order-pay-options").find('input[name="order_payment_method"]')
      .length !== 0
  ) {
    pay_method = $(".dev-order-pay-options")
      .find('input[name="order_payment_method"]:checked')
      .data("type");
  }
  ...
  if (!_this.hasClass("is-disabled")) {
    ...
    $.ajax({
      ...
      data:
        ...
        "&delivery_date=" +
        delivery_date +
        "&action=foodbakery_restaurant_order_confirm",
      dataType: "json",
    })
      .done(function (response) {
        ...
        if (response.is_user_login == false) {
          ...
          jQuery(".wp-user-form").append(
            '<input type="hidden" name="single_restaurant_page" class="single_restaurant_page" value="YES">'
          );
        }
...
$(document).ready(function () {
  ...
  if (jQuery(".contact-area .phone > a").length > 0) {
    ...
    jQuery(".cs-calltoaction.simple a.csborder-color.cs-color").attr(
      "href",
      "#home"
    );
    jQuery(".cs-calltoaction.simple a.csborder-color.cs-color").click(
      function () {
...
function foodbakery_edit_extra_menu_item(
  ...
  jQuery(".update_menu_" + data_rand).after(
    '<img class="update_menu_loader_' +
      data_rand +
      '" src="' +
      plugin_url +
      'assets/frontend/images/ajax-loader.gif" alt="">'
  ...
  jQuery.ajax({
    ...
    dataType: "json",
    data:
      "action=foodbakery_edit_extra_menu_item&popup_id=" +
      popup_id +
      "&data_id=" +
      data_id +
      "&data_cat_id=" +
...
function foodbakery_show_extra_menu_item(
  ...
  jQuery.ajax({
    ...
    dataType: "json",
    data:
      "action=foodbakery_show_extra_menu_item&popup_id=" +
      popup_id +
      "&data_id=" +
      data_id +
      "&data_cat_id=" +
