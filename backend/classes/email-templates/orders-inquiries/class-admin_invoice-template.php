 ...
 * @package	Foodbakery
 */
if (!class_exists('Foodbakery_admin_email_invoice_template')) {

    class Foodbakery_admin_email_invoice_template {
        ...
        public function __construct() {
            ...
            $this->email_template_type = 'Admin invoice';

            $this->email_default_template = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/></head><body style="margin: 0; padding: 0;"><div style="background-color: #eeeeef; padding: 50px 0;"><table style="max-width: 640px;" border="0" cellspacing="0" cellpadding="0" align="center"><tbody><tr><td style="padding: 40px 30px 30px 30px;" align="center" bgcolor="#33333e"><h1 style="color: #fff;">Admin invoice</h1></td></tr><tr><td style="padding: 40px 30px 40px 30px;" bgcolor="#ffffff"><table border="0" width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td valign="top" width="260"><table border="0" width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td style="padding-bottom: 8px;">Invoice for : [ORDER_LINK]</td></tr><tr><td style="padding-bottom: 8px;">[ORDER_USER_NAME] had submitted an order on restaurant ( <a href="[RESTAURANT_LINK]">[RESTAURANT_NAME]</a> ).</td></tr><tr><td>You can overview complete Invoice Report Below including taxes :</td></tr><tr><td>Invoice Detail:</td></tr><tr><td>[ORDER_DETAIL]</td></tr></tbody></table></td></tr></tbody></table></td></tr><tr><td style="background-color: #ffffff; padding: 30px 30px 30px 30px;"><table border="0" width="100%" cellspacing="0" cellpadding="0"><tbody><tr><td style="font-family: Arial, sans-serif; font-size: 14px;">® [SITE_NAME], 2019</td></tr></tbody></table></td></tr></tbody></table></div></body></html>';
            ...
            $this->email_template_index = 'admin-invoice-template';
            ...
            add_action('foodbakery_admin_invoice_email', array($this, 'foodbakery_admin_invoice_email_callback'), 12, 1);
        }

        public function foodbakery_admin_invoice_email_callback($order_id = '') {
            ...
            if (isset($template['email_notification']) && $template['email_notification'] == 1) {
                ...
                $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : esc_html__('Admin invoice', 'foodbakery');
        ...
        function get_order_detail() {
            ...
            if ($order_type == 'order' && is_array($order_menu_list)) {
                ...
                $html .= '<div class="user-order-holder 6">';
                ...
                foreach ($order_menu_list as $_menu_list) {
                    ...
                    if (is_array($extras_item) && sizeof($extras_item) > 0) {
                        ...
                        foreach ($extras_item as $extra_item) {
                            ...
                            $sa_categor_price += floatval($price_extra_item * $price_extra_quantity);
                        }
                        $html_temp .= '</ul>';
                    }
                    $html .= '<span class="category-price">' . foodbakery_get_currency($sa_categor_price, true, '', '', true) . '</span>';
                    $html .= $html_temp;
                    $html = '</li>';
                }
                $html .= '
                ...
                if ($order_m_total > 0) {
                    ...
                    $html .= apply_filters('restaurant_order_send_to_admin', '', $order_id);
                    ...
                    $html .= '</ul>';
                    if ($check_addon) {
                        $html .= apply_filters('restaurant_order_calculation_for_admin', '', $order_id, $order_m_total);
                    } else {
    ...
    new Foodbakery_admin_email_invoice_template();
}
